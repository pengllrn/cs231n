...
class KNearestNeighbor(object):
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in range(num_test):
      ...
      closest_y = []
      distances=dists[i,:]
      indexes = np.argsort(distances)  #由小到大排序，然后依次输出排序后的数原来的索引
      closest_y = self.y_train[indexes[:k]]  #最小的k个索引
      ...
      #bincount:统计array整型数组，从0到array数组中最大元素，依次出现的频率
      #[2,2,5] ===>[0,0,2,0,0,1] 表示这个数组中0，1，3，4出现了0次，2出现了2次
      y_pred[i]=np.argmax(count)#返回最大值的索引
      #########################################################################
      #                           END OF YOUR CODE                            # 
      #########################################################################
